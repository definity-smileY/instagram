...
class UserCreateView(BaseView):
    ...
    def post(self, request):
        # Restful 하게 하기 위해선 POST, DELETE 다 나누어야 하지만, 편의상 POST만 사용하도록 함
        username = request.POST.get('username', '')
        if not username:
            return self.response(message='아이디를 입력해주세요.', status=400)
        password = request.POST.get('password', '')
        if not password:
            return self.response(message='패스원드를 입력해주세요.', status=400)
...
class UserLoginView(BaseView):
    def post(self, request):
        ...
        user = authenticate(request, username=usernmae, password=password)
        if user is None:
